...
def simulate_hospital(
    ...
    """Adapt the shared site-configured simulator to the legacy container."""

    generated = TwoMechanismSimulator(
        config,
        hospital.renal_effect_nt_sd,
        site=hospital,
    ).simulate(rng, patient_count)
    observed = generated.observed
    truth = generated.truth
    if not isinstance(observed, MultiHospitalCohort) or not isinstance(
        truth, TransportSimulationTruth
    ):
        raise RuntimeError("Hospital simulation returned incompatible data.")

    return HospitalCohort(
        raw_biomarkers=observed.biomarkers,
        biomarker_observed=observed.measurement_indicators,
        renal_dysfunction=observed.covariate("renal_dysfunction"),
        background_inflammation=observed.covariate("background_inflammation"),
        true_mechanism=truth.mechanism,
        complete_calibrated_biomarkers=truth.complete_calibrated_biomarkers,
        hospital=hospital,
    )
