...
CONFOUNDING_STYLES = {
    "Associative latent class model": {"color": "#C25A2E", "linestyle": "--", "marker": "s"},
    "Renal-adjusted associative latent class model": {"color": "#D18A3D", "linestyle": "--", "marker": "^"},
    "Biologically constrained latent SCM": {"color": "#1C3F5F", "linestyle": "-", "marker": "o", "filled": True},
    "Data-generating oracle (reference)": {"color": "#6E7880", "linestyle": ":", "marker": "v"},
}
CONFOUNDING_SHORT = {
    "Associative latent class model": "Pooled associative LCM",
    "Renal-adjusted associative latent class model": "Renal-adjusted LCM",
    "Biologically constrained latent SCM": "Biology-constrained SCM",
    "Data-generating oracle (reference)": "Bayes-optimal oracle",
}
...
def plot_confounding_sweep(
    ...
    path: Path,
    *,
    title: str = "Renal status as a confounder of mechanism and marker",
    subtitle: str = "",
    source: str = "",
) -> None:
    """Accuracy, raw false-attribution, and excess over Bayes across confounding.

    Panel c is the one that changes how the other two are read.  Once renal
    status predicts the mechanism, part of a "false atrial" call is *correct*
    Bayesian behaviour — the Bayes-optimal rule makes those calls too.  Plotting
    each model's excess over the oracle separates avoidable error from the
    irreducible kind, which the raw rate silently conflates.
    """

    levels = sorted(raw["atrial_probability_if_renal_impaired"].unique())
    oracle = "Data-generating oracle (reference)"
    figure, axes = plt.subplots(1, 3, figsize=(15.0, 4.5))

    def series(method: str, metric: str) -> tuple[list[float], list[float], list[float]]:
        means, lows, highs = [], [], []
        for level in levels:
            cell = raw[
                (raw["method"] == method)
                & (raw["atrial_probability_if_renal_impaired"] == level)
            ][metric].to_numpy(dtype=float)
            mean, low, high = _mean_ci(cell)
            means.append(100 * mean), lows.append(100 * low), highs.append(100 * high)
        return means, lows, highs

    for axis, metric, ylabel, panel in (
        (axes[0], "accuracy", "Mechanism assignment accuracy (%)", "a"),
        (axes[1], "false_atrial_renal_competing",
         "False atrial rate, renal + competing (%)", "b"),
    ):
        ends = []
        for method, style in CONFOUNDING_STYLES.items():
            style = dict(style)
            filled = bool(style.pop("filled", False))
            color = str(style["color"])
            means, lows, highs = series(method, metric)
            axis.plot(levels, means, linewidth=2.0, markersize=6,
                      markerfacecolor=color if filled else "white",
                      markeredgecolor=color, markeredgewidth=1.4, **style)
            axis.fill_between(levels, lows, highs, color=color, alpha=0.13, linewidth=0)
            ends.append((means[-1], CONFOUNDING_SHORT[method], color))
        axis.set_xlim(levels[0], levels[-1] + 0.135)
        _place_end_labels(axis, ends, levels[-1], gap_fraction=0.075, fontsize=7.5)
        axis.set_ylabel(ylabel)
        axis.set_title(f"{panel}   {'Accuracy' if panel=='a' else 'Raw false attribution'}",
                       fontsize=11, fontweight="bold", loc="left")

    # Panel c — excess over the Bayes-optimal rate.
    ends = []
    for method, style in CONFOUNDING_STYLES.items():
        if method == oracle:
            continue
        style = dict(style); style.pop("filled", None)
        color = str(style["color"])
        block = contrasts[
            (contrasts["comparator"] == method)
            & (contrasts["metric"] == "false_atrial_renal_competing")
        ]
        if block.empty:
            continue
        # contrasts are causal-minus-comparator; invert to comparator-minus-causal,
        # then add the causal model's own excess over the oracle.
        causal_excess = contrasts[
            (contrasts["comparator"] == oracle)
            & (contrasts["metric"] == "false_atrial_renal_competing")
        ].set_index("atrial_probability_if_renal_impaired")["mean_difference"]
        values = []
        for level in levels:
            base = float(causal_excess.get(level, np.nan))
            row = block[block["atrial_probability_if_renal_impaired"] == level]
            delta = float(row["mean_difference"].iloc[0]) if not row.empty else np.nan
            values.append(100 * (base - delta))
        axes[2].plot(levels, values, linewidth=2.0, markersize=6,
                     markerfacecolor="white", markeredgecolor=color,
                     markeredgewidth=1.4, **style)
        ends.append((values[-1], CONFOUNDING_SHORT[method], color))
    causal_only = contrasts[
        (contrasts["comparator"] == oracle)
        & (contrasts["metric"] == "false_atrial_renal_competing")
    ].sort_values("atrial_probability_if_renal_impaired")
    axes[2].plot(causal_only["atrial_probability_if_renal_impaired"],
                 100 * causal_only["mean_difference"], linewidth=2.0, marker="o",
                 color="#1C3F5F", markersize=6)
    axes[2].axhline(0, color="#22282E", linewidth=0.8, linestyle="--")
    axes[2].set_xlim(levels[0], levels[-1] + 0.135)
    _place_end_labels(axes[2], ends, levels[-1], gap_fraction=0.075, fontsize=7.5)
    axes[2].set_ylabel("Excess over Bayes-optimal rate (pp)")
    axes[2].set_title("c   Avoidable error only", fontsize=11, fontweight="bold", loc="left")

    for axis in axes:
        axis.set_xlabel("P(atrial mechanism | kidneys impaired)")
        axis.set_xticks(levels, [f"{v:.2f}" for v in levels])
        axis.yaxis.grid(True, color="#DCE1E5", linewidth=0.8)
        axis.set_axisbelow(True)
        axis.spines[["top", "right"]].set_visible(False)
    axes[0].annotate("no confounding\n(locked setting)", (levels[0], 60),
                     xytext=(4, 0), textcoords="offset points",
                     fontsize=7.5, color="#42474D", va="center")

    figure.suptitle(title + (f"\n{subtitle}" if subtitle else ""),
                    x=0.008, ha="left", fontsize=12.5, y=1.06)
    figure.tight_layout()
    _save(figure, path, source)
